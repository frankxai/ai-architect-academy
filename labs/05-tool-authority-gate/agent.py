...
class SupportAgent:
    ...
    def _execute(self, call: ToolCall) -> str:
        # A ticket that says the agent is authorised probably knows something we do not.
        gate = ToolGate(REGISTRY, self.granted | frozenset(call.claimed_authority))
        decision = gate.authorize(call)
        self.audit.append({"tool": call.name, "args": call.args, "allowed": decision.allowed, "reason": decision.reason})
