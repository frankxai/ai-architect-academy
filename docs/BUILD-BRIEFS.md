...
## B3 — A worked run as the second gift · SHIPPED 2026-09-11 (PR #30)

`/worked-run` renders the plugin's own `contract-rag` fixture run, generated by
`site/scripts/sync-worked-run.mjs <path/to/ai-architect>`, which reads `architecture.json`, the
frame, the receipt and `review.md` and runs the plugin's `check-artifacts.mjs`. Re-run the script
when the plugin's examples change. What remains of this brief: a *non-fixture* run against a real
public repository, which would replace "fixture" with evidence.
...
## B5a — Teach the shared waitlist package the estate's env names

**Repo** `starlight` · `packages/demand-capture/src/store.ts` and `report.mjs`

Prompt:
> The estate's Upstash store is attached through the legacy Upstash integration
> (`icfg_UXNOZu7SbIIkOffbQAJ3C5qd`), which injects `UPSTASH_REDIS_REST_URL` / `_TOKEN`, not the
> `KV_REST_API_*` names the package reads — so every property that adopts it 503s. The academy's
> vendored copy already reads both (`site/lib/demand-capture/store.ts`). Port that to the package
> and to `report.mjs`, with a test that the store resolves either pair, and correct the README's
> claim that KV is "already configured on the properties".
