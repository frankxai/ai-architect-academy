// Lightweight client-side search overlay for docs
(function(){
  ...
  overlay.innerHTML = `
    <div class="w-full max-w-2xl rounded-xl bg-white dark:bg-slate-900 border dark:border-slate-700 shadow-lg">
      <div class="flex items-center gap-2 p-3 border-b dark:border-slate-700">
        <input id="search-input" type="text" placeholder="Search pages, topics..." autofocus
  ...
  function render(q){
    ...
    const items = [...pages, ...sections].filter(it => {
    ...
    results.innerHTML = items.map(it => `
      ...
      </a>`).join('') || '<div class="p-3 text-sm text-slate-500">No results</div>';
  }
